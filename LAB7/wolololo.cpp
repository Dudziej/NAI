...
auto hill_climbing_alg_random = [](auto goal, int max_iterations, auto callback, vector<pair<double, double>>kwadrat) {
    auto init = [&kwadrat]() { //funkcja losująca randomowy punkt z dziedziny
        vector<double> x(kwadrat.size());
        for(int i = 0; i < kwadrat.size(); i++){
        uniform_real_distribution<double> uniform_dist(kwadrat[i].first, kwadrat[i].second);
        x[i] = uniform_dist(e1);
        }return x;
    };
    auto current_solution = init(); //losowanie punktu startu
    for (int iteration = 0; iteration < max_iterations; iteration++)
    {
        auto previous_solutions = current_solution;
        auto next_solutions = init();
        if(goal(next_solutions)< goal(current_solution)){
...
int main(int argc, char **argv)
{
    ...
    {
        cerr << "hcr" << endl;
        vector<pair<double, double>> kwadrat;
        kwadrat.push_back(pair<double, double>(a, b));
        kwadrat.push_back(pair<double, double>(a, b));
